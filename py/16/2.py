...
def countEnergized(startX, startY, startDir):
    q: deque[tuple[int, int, int]] = deque()
    states = set()
    q.append((startX, startY, startDir))
...
for y in range(height):
    m = max(m, countEnergized(0, y, 2))
    m = max(m, countEnergized(width-1, y, 0))

for x in range(width):
    m = max(m, countEnergized(x, 0, 3))
    m = max(m, countEnergized(x, height-1, 1))
